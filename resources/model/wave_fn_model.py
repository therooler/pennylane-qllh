...
class WavefunctionModel(tf.keras.Model):
    """
    ...
    def __init__(self, nclasses, dev_name):
        ...
        self.model_dev = qml.device(dev_name, wires=self.req_qub_in)
...
class WavefunctionModelWrapper:
    """
    QML model training
    """
    ...
    def __init__(self, model: WavefunctionModel):
        """
        This wrapper allows one to minimize the quantum log-likelihood for a given variational quantum circuit.
    ...
    def _determine_req_measurements(self, complex=False) -> None:
        ...
        for m in measurements:
            obs = 1
            for ob in m:
                obs = np.kron(obs, WavefunctionModelWrapper.measurements[ob])
    ...
    def loss(self, inputs: np.ndarray, eta: tf.Tensor) -> tf.float64:
        ...
        # Todo: split self.w here over the observables.
        obs = tf.map_fn(
